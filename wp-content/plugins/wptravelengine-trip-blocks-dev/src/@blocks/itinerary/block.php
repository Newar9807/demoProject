...
if ( $render->is_editor() && 'trip' !== $wtetrip->post->post_type ) {
	...
	$advanced_itinerary          = isset( $advanced_itinerary_settings['advanced_itinerary'] ) ? $advanced_itinerary_settings['advanced_itinerary'] : '';
	$itineraries                 = array();
...
if ( ! isset( $itineraries[0] ) ) {
	?>
	<div class="notice">
		<?php echo esc_html__( 'Please add some itinerary data for this trip.' ); ?>
	</div>
	<?php
} else {
	?>
	<div <?php echo $attributes_parser->wrapper_attributes(); ?> >
		<div class="post-data itinerary wte-trip-itinerary-v2">
			<?php foreach ( $itineraries as $index => $itinerary ) : ?>
				<div class="itinerary-row advanced-itinerary-row <?php echo ( $enabled_expand_all || 0 === $index ) ? 'active' : ''; ?>">
						...
						<div class="title">
							...
							if ( isset( $itinerary['days_label'] ) && ! empty( $itinerary['days_label'] ) ) {
								...
								if ( true === $attributes_parser->get( 'dayLabel' ) ) {
									...
									echo sprintf( __( 'Day %s : ', 'wp-travel-engine' ), esc_attr( $index + 1 ) );
								}
							}
							?>
							</span>
					...
					<div class="itinerary-content <?php echo ( $enabled_expand_all || 0 === $index ) ? 'show' : ''; ?>" <?php echo ( $enabled_expand_all || 0 === $index ) ? 'style="display: block;"' : ''; ?>>
						...
						if ( defined( 'WTEAI_VERSION' ) ) {
									...
									if ( isset( $itinerary['meals_included'] ) && is_array( $itinerary['meals_included'] ) ) {
										...
										<div class="itinerary-meals">
											...
											$iti_meals_array  = apply_filters(
												...
												array(
													'breakfast' => __( 'Breakfast', 'wptravelengine-trip-blocks' ),
													'lunch'  => __( 'Lunch', 'wptravelengine-trip-blocks' ),
													'dinner' => __( 'Dinner', 'wptravelengine-trip-blocks' ),
												)
											);
									...
									if ( isset( $itinerary['sleep_modes'] ) && ! empty( $itinerary['sleep_modes'] ) ) {
										...
										<div class="itinerary-sleep-mode">
											...
											if ( isset( $itinerary['sleep_mode_description'] ) && '' !== $itinerary['sleep_mode_description'] ) {
												...
												echo function_exists( 'wptravelengine_svg_by_fa_icon' ) ? wptravelengine_svg_by_fa_icon( 'fas fa-info' ) : '<i class="fas fa-info-circle';
												echo '</span></a>';
											} else {
							...
							if ( isset( $itinerary['sleep_mode_description'] ) && '' !== $itinerary['sleep_mode_description'] ) {
								?>
								<div class="content-additional-sleep-mode" id="content-additional-sleep-mode-<?php echo $index; ?>" style="display: none;">
									<div class="additional-sleep-mode-inner">
										<a href="javascript:void(0);" class="wte-ai-close-button"><?php _e( 'Close', 'wptravelengine-trip-blocks' ); ?></a>
