...
if ( $render->is_editor() && ! isset( $wtetrip->post->post_type ) ) {
	$packages = SampleData::prices();
	foreach ( $packages as $package ) {
		$prices_data[] = [
			'price'      => $package[ 'price' ],
			'sale_price' => $package[ 'sale_price' ],
			'has_sale'   => true,
			'per_label'  => $package[ 'per_label' ],
		];
	}
} else {
	if ( WP_TRAVEL_ENGINE_POST_TYPE != $wtetrip->post->post_type ) {
		return;
	}
...
<div <?php echo $attributes_parser->wrapper_attributes(); ?>>
    <div class="wpte-bf-price-wrap">
        <?php foreach ($prices_data as $data): ?>
            <div class="wpte-bf-price">
                <span class="wpte-bf-reg-price">
					<?php
					$regular_price = wte_get_formated_price( $data[ 'price' ], '', $attributes_parser->get( 'displayFormat' ) );
					echo wp_kses( str_replace( '%regular_price%', "<del>{$regular_price}</del>", $attributes_parser->get( 'pricePrefix' ) ), array(
						'span' => array(
							'class' => array(),
						),
						'del'  => array(
                ...
                <span class="wpte-bf-offer-price">
					<?php
					$sale_price         = wte_get_formated_price( $data[ 'sale_price' ], '', $attributes_parser->get( 'displayFormat' ) );
